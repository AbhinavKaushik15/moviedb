    ...
    <div className="w-full min-h-screen bg-[#1D1C23] px-[3.5vw] text-white">
      <nav className="text-zinc-400 w-fit font-bold pt-5 flex items-center gap-2">
        <Link to="/" onClick={() => navigate(-1)}>
          <IoMdArrowRoundBack className="hover:text-purple-600 text-[3vh] sm:text-[2vw]" />
        </Link>
      </nav>

      <div className="relative w-full flex justify-between pt-[3vh] sm:pt-[2vw]">
        <div className="relative hero_Section w-[17vw] flex flex-col">
          <img
            className="w-[20vw] h-[25vw] object-cover rounded-md sm:rounded-xl"
            src={`https://image.tmdb.org/t/p/original/${info.detail.profile_path}`}
            alt=""
          />

          <hr className="mt-[2vw] w-[17vw] pt-[1vw]" />
          ...
          <h1 className="text-[2.4vw] mt-[2.2vw] font-[500]">Personal info</h1>

          <h1 className="mt-[2.2vw] font-[500] text-[1.6vw]">Known For</h1>
          <h1 className="text-[1.25vw]">{info.detail.known_for_department}</h1>
          ...
          <h1 className="mt-[2.2vw] font-[500] text-[1.6vw]">Place Of Birth</h1>
          <h1 className="text-[1.25vw]">{info.detail.place_of_birth}</h1>

          <h1 className="mt-[2.2vw] font-[500] text-[1.6vw]">Also Known As</h1>
          <h1 className="text-[1.25vw]">
            {info.detail.also_known_as.join(", ")}
          </h1>
        </div>

        <div className="relative flex flex-col gap-[2vw] w-[70vw]">
          <h1 className="text-[3.5vw] text-white font-[700]">
            {info.detail.name}
          </h1>
          <h1 className="text-[2.1vw] font-[500]">Biography</h1>
          <p className="text-[1.25vw] w-[98%]">{info.detail.biography}</p>

          <div>
            <h1 className="text-[2.1vw] text-zinc-500">Known For</h1>
          ...
          {info[category + "Credits"].cast ? (
            <div className="flex flex-col gap-[4vw] md:gap-[2vw]">
              <div className="flex items-center justify-between">
                <h1 className="text-[2.1vw] text-zinc-500">Acting</h1>
              ...
              <div className="h-[50vh] text-white list-disc shadow-md shadow-zinc-500 overflow-x-hidden overflow-y-auto flex flex-col mb-[4vw]">
                {info[category + "Credits"].cast.map((c, i) => (
                  <Link to={`/${category}/details/${c.id}`}>
                    <li
                      key={i}
                      className="w-full hover:bg-zinc-800 duration-300 pl-3 cursor-pointer py-[1vw]"
                    >
                      <span className="list-disc text-[1.5vw]">
                        {c.name || c.title || c.original_title || original_name}
                      </span>

                      <span className="block ml-6 text-[1.35vw] text-zinc-500">
